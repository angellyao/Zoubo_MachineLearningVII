...
def calc_statistics(x):
    ...
    print('1.手动计算均值、标准差、偏度、峰度：', mu, sigma, skew, kurtosis)
...
if __name__ == '__main__':
    ...
    print('2.函数库计算均值、标准差、偏度、峰度：', mu, sigma, skew, kurtosis)
    # 一维直方图
    ...
    mu, sigma, skew, kurtosis = calc_statistics(d)
    print('3.函数库计算均值、标准差、偏度、峰度：', mu, sigma, skew, kurtosis)

    # 二维图像
    N = 30
    density, edges = np.histogramdd(d, bins=[N, N])
    print('4.样本总数：', np.sum(density))
    density /= density.max()
    x = y = np.arange(N)
    # print('x = ', x)
    # print('y = ', y)
