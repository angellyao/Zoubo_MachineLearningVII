...
def data_clean(show):
    ...
    if show:
        ...
        plt.ylabel('电流强度', fontsize=14)
        plt.title('原始电流的变化情况', fontsize=16)
        plt.show()
...
if __name__ == "__main__":
    ...
    y = data['Current'].values.reshape(-1, 1)
    ...
    plt.figure(num=1, facecolor='w', figsize=(8, 7))
    ...
    for component in np.arange(n_components):
        ...
        plt.ylim((-0.1, 1.1))
        plt.grid(b=True, ls=':')
        plt.ylabel('组份概率', fontsize=14)
        plt.xlabel('时间', fontsize=14)
        plt.title('第%d组份的概率分布' % (component+1), fontsize=16)
    plt.suptitle('原始电流/组份与时间的变化关系', fontsize=18)
    ...
    for component in np.arange(n_components):
        ...
        plt.ylim((-0.1, 1.1))
        plt.grid(b=True, ls=':')
        plt.xlabel('原始电流强度', fontsize=14)
        plt.ylabel('组份概率', fontsize=14)
        plt.title('第%d组份的概率分布' % (component + 1), fontsize=16)
    plt.suptitle('各组份的概率分布', fontsize=18)
    plt.tight_layout(pad=1, rect=(0, 0, 1, 0.96))
    plt.savefig('2.png')
    ...
    plt.xlabel('时间', fontsize=14)
    plt.ylabel('电流强度', fontsize=14)
    plt.title('整形后的电流数据', fontsize=18)
    plt.subplot(212)
    plt.plot(data['Current'], data['New'], 'r.')
    plt.ylim(extend(data['New'].min(), data['New'].max()))
    plt.grid(b=True, ls=':')
    plt.xlabel('原始电流强度', fontsize=14)
    plt.ylabel('修正后的电流强度', fontsize=14)
    plt.title('整形前后的电流关系', fontsize=18)
